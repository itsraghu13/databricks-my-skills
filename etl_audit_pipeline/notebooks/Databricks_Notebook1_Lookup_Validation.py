...
# MAGIC %md
# MAGIC # Notebook1 - ETL Audit Lookup and Validation (Databricks Unity Catalog)
# MAGIC 
# MAGIC **Catalog:** dev_edf_silver  
# MAGIC **Schema:** dps_stage  
# MAGIC 
# MAGIC **Purpose:**
...
dbutils.widgets.text("src_table1", "", "Source Table (full path: catalog.schema.table)")
...
ref_table = f"{catalog_name}.{schema_name}.ETL_AUDT_REF"
fact_table = f"{catalog_name}.{schema_name}.ETL_AUDT_FACT"
...
def lookup_etl_audt_ref(job_name, src_table1, ref_table):
...
ref_data = lookup_etl_audt_ref(job_name, src_table1, ref_table)
if ref_data is None:
...
def get_unprocessed_job_run_items(fact_table):
...
unprocessed_items, unprocessed_count = get_unprocessed_job_run_items(fact_table)
...
def get_job_run_items_from_source(src_table1, unprocessed_items, unprocessed_count):
    ...
    try:
        if unprocessed_count > 0:
            ...
            source_df = spark.table(src_table1)
...
def insert_into_etl_audt_fact(job_run_items_df, fact_table):
    """Insert new records into ETL_AUDT_FACT with Processed_Flag='N'"""
    try:
        insert_df = job_run_items_df.withColumn("processed_flag", lit("N")) \
            .withColumn("run_status", lit(None).cast("string")) \
            .withColumn("job_start_time", lit(None).cast("timestamp")) \
            .withColumn("job_end_time", lit(None).cast("timestamp")) \
            .withColumn("source_count", lit(None).cast("long")) \
            .withColumn("target_count", lit(None).cast("long")) \
            .withColumn("error_count", lit(0)) \
            .withColumn("error_message", lit(None).cast("string")) \
            .withColumn("batch_no", lit(None).cast("int")) \
            .withColumn("notebook_name", lit(None).cast("string")) \
            .withColumn("create_ts", lit(None).cast("timestamp")) \
            .withColumn("update_ts", current_timestamp())
        
        insert_count = insert_df.count()
        if insert_count > 0:
            insert_df.write.mode("append").saveAsTable(fact_table)
...
inserted_count = insert_into_etl_audt_fact(job_run_items_df, fact_table)
